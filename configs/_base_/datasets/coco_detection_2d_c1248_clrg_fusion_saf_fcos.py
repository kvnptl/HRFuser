...
train_pipeline = [
    ...
    dict(type='Crop', crop_size=(768, 1280), offsets=(202, 280), skip_keys=['radar_img']),
    dict(type='Resize', img_scale=(1280, 768), keep_ratio=False, skip_keys=['radar_img']),
    ...
    dict(type='RandomDrop', p = [0.5, 0.5, 0.5, 0.5], keys=['img', 'radar_img']),
...
test_pipeline = [
    ...
    dict(
        ...
        transforms=[
            ...
            # dict(type='Normalize', **gated_norm_cfg, keys=['gated_img'], sensor_type='gated'),
            dict(type='Crop', crop_size=(768, 1280), offsets=(202, 280), skip_keys=['radar_img']),
            dict(type='Resize', keep_ratio=False, skip_keys=['radar_img']),
...
data = dict(
    ...
    test=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file= 'coco_annotations/dense_infos_test_clear_coco.json',
        img_prefix='',
        lidar_prefix='',
        radar_prefix='',
        lidar_img_mode=False,
