...
train_pipeline = [
    ...
    dict(type='Resize', img_scale=(640, 360), keep_ratio=True, skip_keys=['lidar_img']),
    dict(type='RandomFlip', flip_ratio=0.5),
    dict(type='Normalize', **img_norm_cfg, keys=['img']),
    dict(type='Pad', size_divisor=32),
    dict(type='RandomDrop', p = [0.0, 0.0, 0.0], keys=['img', 'lidar_img']),
    dict(type='DefaultFormatBundle', sensor_keys=['img', 'lidar_img']),
    dict(type='Collect', keys=['img', 'lidar_img','gt_bboxes', 'gt_labels'],
    meta_keys=('filename', 'ori_filename', 'ori_shape',
                            'img_shape', 'pad_shape', 'scale_factor', 'flip',
                            'flip_direction', 'img_norm_cfg', 'lidar_ori_shape', 'lidar_norm_cfg')),
...
test_pipeline = [
    ...
    dict(
        ...
        transforms=[
            ...
            dict(type='Resize', keep_ratio=True, skip_keys=['lidar_img']),
            dict(type='RandomFlip'),
            dict(type='Normalize', **img_norm_cfg, keys=['img']),
            dict(type='Pad', size_divisor=32),
            dict(type='ImageToTensor', keys=['img', 'lidar_img']),
            dict(type='Collect', keys=['img', 'lidar_img']),
        ])
]
data = dict(
    samples_per_gpu=1,
