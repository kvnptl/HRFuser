...
train_pipeline = [
    ...
    dict(type='Resize', img_scale=(640, 360), keep_ratio=True, skip_keys=['radar_img']),
    dict(type='RandomFlip', flip_ratio=0.5),
    dict(type='Normalize', **img_norm_cfg, keys=['img']),
    dict(type='Pad', size_divisor=32),
    dict(type='RandomDrop', p = [0.2, 0.2, 0.2], keys=['img', 'radar_img']),
    dict(type='DefaultFormatBundle', sensor_keys=['img', 'radar_img']),
    dict(type='Collect', keys=['img', 'radar_img','gt_bboxes', 'gt_labels'],
    meta_keys=('filename', 'ori_filename', 'ori_shape',
                            'img_shape', 'pad_shape', 'scale_factor', 'flip',
                            'flip_direction', 'img_norm_cfg', 'radar_ori_shape', 'radar_norm_cfg')),
...
test_pipeline = [
    ...
    dict(
        ...
        transforms=[
            ...
            dict(type='Resize', keep_ratio=True, skip_keys=['radar_img']),
            dict(type='RandomFlip'),
            dict(type='Normalize', **img_norm_cfg, keys=['img']),
            dict(type='Pad', size_divisor=32),
            dict(type='ImageToTensor', keys=['img', 'radar_img']),
            dict(type='Collect', keys=['img', 'radar_img']),
        ])
]
data = dict(
    samples_per_gpu=1,
    ...
    train=dict(
        ...
        classes=classes,
        data_root=data_root,
        ann_file='nuscenes_infos_train_mono3d_saf_fcos_with_SAF_FCOS_ANNOTATIONS.coco.json',
        img_prefix='',
        lidar_prefix='',
        radar_prefix='',
    ...
    val=dict(
        ...
        classes=classes,
        data_root=data_root,
        ann_file='nuscenes_infos_val_mono3d_saf_fcos_with_SAF_FCOS_ANNOTATIONS.coco.json',
        img_prefix='',
        lidar_prefix='',
        radar_prefix='',
        lidar_img_mode=False,
